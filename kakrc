...
map global user G ":grep-selection-linked<ret>"-docstring "Grep the selection or the word under the cursor in an alternate project"
...
def -params 1..1 to-buffer %{
    %sh{
        buffname=$(echo $kak_bufflist | awk -F ':' -v idx=$1 'idx < NF {print $(NF-idx)}')
...
def -hidden grep-selection-linked %{
    %sh{
        if [ $(echo $kak_selection_desc | sed 's/,/\n/' | sort -u | wc -l ) == "1" ]; then
            echo "exec <a-i>w:grep-linked<space><c-r>.<ret>"
        else
            echo "exec :grep-linked<space>\"<c-r>.\"<ret>"
        fi
    }
}
...
set global ycmd_path /usr/share/vim/vimfiles/third_party/ycmd/ycmd
